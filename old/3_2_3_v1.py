...
creator.create("FitnessMax", base.Fitness, weights=(-1.0,)) #適応度の定義
creator.create("Individual", np.ndarray, fitness=creator.FitnessMax) #個体の定義
...
def evalOneMax(individual): #評価関数の計算
    ...
    t21 = p2[fin_seq1[0]] #処理時刻の入れ物
    t22 = p2[fin_seq1[1]] #処理時刻の入れ物
    c21 = fin_time1[fin_seq1[0]] #装置1の開始時刻
    c22 = fin_time1[fin_seq1[1]] #装置2の開始時刻
    ...
    for i in range(2,n_gene):
        max_time = max(min(c21+t21,c22+t22),fin_time1[fin_seq1[i]]) #次のジョブが処理可能になる時刻
        ...
        if max_time == c21+t21:
            ...
            max_sub_time = min(c21+t21,c22+t22)
